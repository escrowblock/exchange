...
(function($) {
    ...
    $.tablesort.prototype = {
        ...
        sort(th, direction) {
            ...
            const rows = rowsContainer.find('tr').has('td, th');
            const cells = rows.find(`:nth-child(${th.index() + 1})`).filter('td, th');
            const { sortBy } = th.data();
            const sortedMap = [];
            ...
            setTimeout(function() {
                ...
                for (let i = 0, { length } = unsortedValues; i < length; i++) {
                    sortedMap.push({
                        index: i,
                        cell: cells[i],
                        row: rows[i],
