...
global.describe('ME StopLimit', function () {
    this.timeout(10000);
    ...
    /**
     * Order type is StopLimit
    * */
    
    // 1 to 1 simple stop limit
    global.it('test buy stop limit order by 1001 with stop price 900 with deal by 901', function (done) {
        this.timeout(10000);
        
        const orderBuySL = EJSON.clone(orderInstance);
        orderBuySL.UserId = userTaker.userId;
        orderBuySL.LimitPrice = Decimal('1001');
        orderBuySL.StopPrice = Decimal('900');
        orderBuySL.Quantity = Decimal('3.145');
        orderBuySL.OrigQuantity = Decimal('3.145');
        orderBuySL.Side = 'Buy';
        orderBuySL.OrderType = 'StopLimit';
        
        const buyIdSL = order.insert(orderBuySL);
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                const buyIdSL_new = order.findOne({ PreviousOrderRevision: buyIdSL })._id;

                chai.assert.equal(trade.find({ OrderId: { $in: [buyId, buyIdSL_new, sellId, sellId2] } }).count(), 4);
                ...
                chai.assert.equal(trade.findOne({ OrderId: buyIdSL_new }).RemainingQuantity.toString(), '0', 'buySL RemainingQuantity is 0');
                ...
                chai.assert.equal(order.findOne({ _id: buyIdSL }).OrderState, 'Rejected', 'BuySL Algo state is Rejected');
                chai.assert.equal(order.findOne({ _id: buyIdSL_new }).QuantityExecuted.toString(), '3.145', 'buySL QuantityExecuted is 3.145');
    ...
    global.it('test sell stop limit order by 1001 with stop price 1101 with deal by 1100', function (done) {
        this.timeout(10000);
