...
global.describe('ME Limit order', function () {
    this.timeout(10000);
    ...
    * */
    ...
    // 1 to 1 simple limit
    global.it('test equal limit orders by 1001 both', function (done) {
        this.timeout(10000);
    ...
    global.it('test gt limit orders by 1001 both', function (done) {
        this.timeout(10000);
    ...
    global.it('test lt limit orders by 1001 both', function (done) {
        this.timeout(10000);
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                ...
                chai.assert.equal(trade.findOne({ OrderId: buyId }).RemainingQuantity.toString(), '0', 'buy RemainingQuantity is 0');
                chai.assert.equal(trade.findOne({ OrderId: sellId }).RemainingQuantity.toString(), '96.855', 'sell RemainingQuantity is 96.855');
                
                chai.assert.equal(order.findOne({ _id: buyId }).Quantity.toString(), '0', 'buy Quantity is 0');
                chai.assert.equal(order.findOne({ _id: sellId }).Quantity.toString(), '96.855', 'sell Quantity is 96.855');
    ...
    // 2 to 1 simple limit
    global.it('test equal limit orders by 1001 both, 2 buy and one sell', function (done) {
        this.timeout(10000);
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                ...
                chai.assert.equal(order.findOne({ _id: buyId }).Quantity.toString(), '0', 'buy Quantity is 0');
                chai.assert.equal(order.findOne({ _id: buyId2 }).Quantity.toString(), '0', 'buy Quantity is 0');
                chai.assert.equal(order.findOne({ _id: sellId }).Quantity.toString(), '0', 'sell Quantity is 0');
                
                chai.assert.equal(order.findOne({ _id: buyId }).QuantityExecuted.toString(), '3.145', 'buy QuantityExecuted is 3.145');
                chai.assert.equal(order.findOne({ _id: buyId2 }).QuantityExecuted.toString(), '3.145', 'buy QuantityExecuted is 3.145');
    ...
    global.it('test gt limit orders by 1001 both, 2 sell and 1 buy', function (done) {
        this.timeout(10000);
    ...
    global.it('test lt limit orders by 1001 both, 2 buy and 1 sell', function (done) {
        this.timeout(10000);
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                ...
                chai.assert.equal(trade.findOne({ OrderId: buyId }, { sort: { ExecutionTime: -1 } }).RemainingQuantity.toString(), '0', 'buy RemainingQuantity is 0');
                chai.assert.equal(trade.findOne({ OrderId: buyId2 }, { sort: { ExecutionTime: -1 } }).RemainingQuantity.toString(), '0', 'buy2 RemainingQuantity is 0');
                chai.assert.equal(trade.findOne({ OrderId: sellId }, { sort: { ExecutionTime: -1 } }).RemainingQuantity.toString(), '93.71', 'sell RemainingQuantity is 93.71');
                
                chai.assert.equal(order.findOne({ _id: buyId }).Quantity.toString(), '0', 'buy Quantity is 0');
                chai.assert.equal(order.findOne({ _id: buyId2 }).Quantity.toString(), '0', 'buy2 Quantity is 0');
                chai.assert.equal(order.findOne({ _id: sellId }).Quantity.toString(), '93.71', 'sell Quantity is 93.71');
