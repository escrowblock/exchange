...
const nodemailer = Npm.require('node4mailer');
...
const makeTransport = function (mailUrlString) {
    ...
    const credentials = new String(mailUrl.auth).split(':');
    const transport = nodemailer.createTransport(
        {
            secure: mailUrl.protocol == 'smtps:',
            port: mailUrl.port,
            host: mailUrl.hostname,
            auth: {
                user: credentials[0],
                pass: credentials[1],
            },
            pool: mailUrl.query.pool,
            tls: {
...
Email.send = function (options) {
    for (let i = 0; i < sendHooks.length; i++) if (!sendHooks[i](options)) return;
