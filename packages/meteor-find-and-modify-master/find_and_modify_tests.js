(function() {
    const TestCollection = new Mongo.Collection('findAndModifyTestCol');
    if (Meteor.isServer) {
   ...
   */
    const addTest = function(name, method, assertions) {
        const methodObj = {};
        let returnVal;
    ...
    addTest('findAndModify - find + set',
        function() {
            TestCollection.remove({});
            const batmanId = TestCollection.insert({
                name: 'Batman',
                favoriteColor: 'black',
            });
            const supermanId = TestCollection.insert({
                name: 'Superman',
                favoriteColor: 'blue',
            });
    ...
    addTest('findAndModify - remove',
        function() {
            ...
            const findAndModifyResponse = TestCollection.findAndModify({
                query: { name: 'Batman' },
                remove: true,
            });
            const count = TestCollection.find({}).count();
            const batman = TestCollection.findOne({ name: 'Batman' });
            return {
                findAndModifyResponse,
                count,
                batman,
            };
        },
    ...
    addTest('findAndModify - positional operators',
        function() {
            TestCollection.remove({});
            const justiceLeague = TestCollection.insert({
                heroes: [
            ...
            const avengers = TestCollection.insert({
                heroes: [
