        ...
        slimScroll(h) {
            const a = f.extend({
                ...
                function r(d) {
                    if (s) {
                        d = d
|| window.event; let c = 0; d.wheelDelta && (c = -d.wheelDelta / 120); d.detail && (c = d.detail / 3); f(d.target || d.srcTarget || d.srcElement).closest(`.${a.wrapperClass}`).is(b.parent()) && m(c, !0); d.preventDefault && !k && d.preventDefault(); k || (d.returnValue = !1);
                    }
                } function m(d, f, h) {
                    k = !1; let e = d; const g = b.outerHeight() - c.outerHeight(); f && (e = parseInt(c.css('top')) + d * parseInt(a.wheelStep) / 100 * c.outerHeight(), e = Math.min(Math.max(e, 0), g), e = d > 0 ? Math.ceil(e) : Math.floor(e), c.css({ top: `${e}px` })); l = parseInt(c.css('top')) / (b.outerHeight() - c.outerHeight());
                    ...
                    a.height = a.height == 'auto' ? b.parent().height() : a.height; n = f('<div></div>').addClass(a.wrapperClass).css({
                        position: 'relative',
                        overflow: 'hidden',
                        width: a.width,
                        height: a.height,
                    }); b.css({ overflow: 'hidden', width: a.width, height: a.height }); var g = f('<div></div>').addClass(a.railClass).css({
                    ...
                    }); var c = f('<div></div>').addClass(a.barClass).css({
                        background: a.color,
                        width: a.size,
                        position: 'absolute',
                        top: 0,
                        ...
                        zIndex: 99,
                    }); var q = a.position == 'right' ? { right: a.distance } : { left: a.distance }; g.css(q); c.css(q); b.wrap(n); b.parent().append(c); b.parent().append(g); a.railDraggable && c.bind('mousedown', function(a) {
                        const b = f(document); y = !0; t = parseFloat(c.css('top')); pageY = a.pageY; b.bind('mousemove.slimscroll', function(a) { currTop = t + a.pageY - pageY; c.css('top', currTop); m(0, c.position().top, !1); });
