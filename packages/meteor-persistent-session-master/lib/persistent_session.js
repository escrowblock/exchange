...
PersistentSession = function (dictName) {
   ...
   */
    function migrateToEJSON() {
        ...
        const psKeyList = amplify.store(`__PSKEYS__${self._dictName}`);
        const psaKeyList = amplify.store(`__PSAKEYS__${self._dictName}`);
    ...
    function migrate3Xto4X() {
        ...
        _.each([psKeyList, psaKeyList], function(list) {
            _.each(list, function(key) {
                ...
                if (!invalid) {
                    const parsed = EJSON.parse(amplify.store(self._dictName + key));
    ...
    if (Meteor.isClient) {
        ...
        Meteor.startup(function() {
            ...
            // persistent data
            const psList = amplify.store(`__PSKEYS__${self._dictName}`);
            ...
            // authenticated data
            const psaList = amplify.store(`__PSAKEYS__${self._dictName}`);
...
PersistentSession.prototype.equals = function _psEquals(key, value) {
    ...
    if (Tracker.active) {
        ...
        if (!_.has(this._dict.keyValueDeps[key], serializedValue)) this._dict.keyValueDeps[key][serializedValue] = new Tracker.Dependency();
        ...
        if (isNew) {
            ...
            Tracker.onInvalidate(function () {
                ...
                if (!that._dict.keyValueDeps[key][serializedValue].hasDependents()) delete that._dict.keyValueDeps[key][serializedValue];
            });
        }
    }
