...
global.describe('ME TrailingStopLimit', function () {
    this.timeout(10000);
    ...
    * */
    ...
    global.it('test StopPprice buy trailing stop limit order by 1001 with stop price 900 with deal by 870 TrailingAmount 50 and PegPriceType Last', function (done) {
        ...
        orderBuySTL.OrderType = 'TrailingStopLimit';
        orderBuySTL.PegPriceType = 'Last';
        orderBuySTL.TrailingAmount = Decimal('50');
        orderBuySTL.TrailingStopLimit = Decimal('10.33');
    ...
    global.it('test Full trades StopPprice buy trailing stop limit order by 1001 with stop price 900 with deal by 870 TrailingAmount 50 and PegPriceType Last', function (done) {
        ...
        orderBuySTL.PegPriceType = 'Last';
        orderBuySTL.TrailingAmount = Decimal('50');
        orderBuySTL.LimitOffset = Decimal('10.33');
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                ...
                chai.assert.equal(order.findOne({ _id: buyIdSTL_new }).QuantityExecuted.toString(), '3.145', 'buySTL QuantityExecuted is 3.145');
                chai.assert.equal(order.findOne({ _id: buyIdSTL_new }).Quantity.toString(), '0', 'buySTL Quantity is 3.145');
                chai.assert.equal(order.findOne({ _id: buyIdSTL_new }).LimitPrice.toString(), '930.33', 'buySTL LimitPrice is 930.33');
                
                chai.assert.equal(order.findOne({ _id: sellId3 }).QuantityExecuted.toString(), '3.145', 'sell3 QuantityExecuted is 3.145');
                chai.assert.equal(order.findOne({ _id: sellId3 }).Quantity.toString(), '0', 'sell3 Quantity is 0');
                done();
            } catch (e) {
                done(e);
            }
    ...
    global.it('test StopPprice sell trailing stop limit order by 1001 with stop price 900 with deal by 970 TrailingAmount 50 and PegPriceType Last', function (done) {
        ...
        orderSellSTL.OrderType = 'TrailingStopLimit';
    ...
    global.it('test Full trades StopPprice sell trailing stop limit order by 1001 with stop price 900 with deal by 970 TrailingAmount 50 and PegPriceType Last', function (done) {
        ...
        const orderSellSTL = EJSON.clone(orderInstance);
        orderSellSTL.UserId = userMaker.userId;
        orderSellSTL.StopPrice = Decimal('900'); // initial StopPrice
        orderSellSTL.Quantity = Decimal('3.145');
        orderSellSTL.OrigQuantity = Decimal('3.145');
        orderSellSTL.Side = 'Sell';
        ...
        setTimeout(Meteor.bindEnvironment(() => {
            try {
                ...
                chai.assert.equal(order.findOne({ _id: sellIdSTL_new }).QuantityExecuted.toString(), '3.145', 'sellSTL QuantityExecuted is 3.145');
                chai.assert.equal(order.findOne({ _id: sellIdSTL_new }).Quantity.toString(), '0', 'sellSTL Quantity is 0');
                chai.assert.equal(order.findOne({ _id: sellIdSTL_new }).LimitPrice.toString(), '909.67', 'sellSTL LimitPrice is 909.67');
                
                chai.assert.equal(order.findOne({ _id: buyId3 }).QuantityExecuted.toString(), '3.145', 'buy3 QuantityExecuted is 3.145');
                chai.assert.equal(order.findOne({ _id: buyId3 }).Quantity.toString(), '0', 'buy3 Quantity is 0');
                done();
            } catch (e) {
                done(e);
            }
